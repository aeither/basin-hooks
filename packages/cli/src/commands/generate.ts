...
export async function generate(options: Generate) {
  ...
  const configs = Array.isArray(resolvedConfigs)
    ? resolvedConfigs
    : [resolvedConfigs]
  for (const resolvedConfig of configs) {
    if (outNames.has(resolvedConfig.out)) throw new Error('out is not unique')
    outNames.add(resolvedConfig.out)
    logger.log(`Config "${resolvedConfig.out}"`)
