...
```ts
import { estimateGas } from 'basin-hooks'
```
...
```ts
import { type EstimateGasParameters } from 'basin-hooks'
```
...
::: code-group
```ts [index.ts]
import { getConnections, estimateGas } from 'basin-hooks'
import { parseEther } from 'viem'
import { config } from './config'
...
```ts
import { type EstimateGasReturnType } from 'basin-hooks'
```
...
```ts
import { type EstimateGasErrorType } from 'basin-hooks'
```
