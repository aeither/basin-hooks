...
```ts
import { type SimulateContractParameters } from 'basin-hooks'
```
...
::: code-group
```ts [index.ts]
import { getAccount, simulateContract } from 'basin-hooks'
import { abi } from './abi'
import { config } from './config'
...
```ts
import { type SimulateContractReturnType } from 'basin-hooks'
```
...
```ts
import { type SimulateContractErrorType } from 'basin-hooks'
```
