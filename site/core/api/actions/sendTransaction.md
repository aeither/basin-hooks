...
```ts
import { type SendTransactionParameters } from 'basin-hooks'
```
...
::: code-group
```ts [index.ts]
import { getConnections, sendTransaction } from 'basin-hooks'
import { parseEther } from 'viem'
import { config } from './config'
...
```ts
import { type SendTransactionReturnType } from 'basin-hooks'
```
